...
const uploadFood = async (req, res) => {
    try {
        ...
        const { imageUrl, mealType, foodName: providedFoodName, calories: providedCalories, protein: providedProtein, carbs: providedCarbs, fat: providedFat } = req.body;
        ...
        const entry = new FoodEntry({
            userId: user._id,
            foodName: providedFoodName || analysis.foodName || 'Unknown',
            imageUrl,
            calories: providedCalories != null ? Number(providedCalories) : Number(analysis.calories || 0),
            protein: providedProtein != null ? Number(providedProtein) : Number(analysis.protein || 0),
            carbs: providedCarbs != null ? Number(providedCarbs) : Number(analysis.carbs || 0),
            fat: providedFat != null ? Number(providedFat) : Number(analysis.fat || 0),
